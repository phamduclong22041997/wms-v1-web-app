...
class AddressArea {
    const COLLECTION_NAME = "AddressArea";
    ...
    public function loadDocument($data) 
    {
        ...
        if($ref == "") {
            ...
            if(isset($data['isdeleted']) && $data['isdeleted']) {
                $country_collection = MongoDBManager::getCollection(Configuration::COLLECTION_NETWORKING_ADDRESS_COUNTRY, Configuration::$CONNECTION);
                $persist = $country_collection->findOne(['area.areacode' => $ref, 'isdeleted' => 0]);
                if ($persist) {
                    throw new \Exception("mess.isusing");
                }
                $this->ACTION = "DEL";
            }
            $this->document = $this->getCollection()->findOne(['areacode' => $ref, 'isdeleted' => 0]);
        }
    ...
    {
        return MongoDBManager::getCollection(AddressArea::COLLECTION_NAME, Configuration::$CONNECTION);
    }
}