...
class OperationTransaction {
    const COLLECTION_NAME = "ParameterOperationTransaction";
    ...
    public function loadDocument($data) 
    {
        ...
        if($ref == "") {
            ...
            $this->document = $this->getCollection()->findOne(
                [
                    'object.code'               => $filter[0]??"", 
                    'transactiontype.code'      => $filter[1]??"", 
                    'transactioncode'     => $filter[1]??"", 
                    'isdeleted' => 0
                ]);
        }
    ...
    {
        return MongoDBManager::getCollection(OperationTransaction::COLLECTION_NAME, Configuration::$CONNECTION);
    }
}