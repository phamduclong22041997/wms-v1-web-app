...
class WarehouseProperty {
    const COLLECTION_NAME = "WarehouseProperty";
    ...
    private function grantData($data)
    {
        ...
        foreach($this->schema as $key => $type) {
            if(isset($data[$key])) {
                $saveData[$key] = is_string($data[$key]) ? trim($data[$key]) : $data[$key];
            }
        }
        if($this->ACTION == "DEL") {
    ...
    {
        return MongoDBManager::getCollection(WarehouseProperty::COLLECTION_NAME, Configuration::$CONNECTION);
    }
}