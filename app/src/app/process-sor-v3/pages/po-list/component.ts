...
export class ListSORComponent implements OnInit, AfterViewInit {
  ...
  initData() {
    ...
    this.poTypeConfig = {
      ...
      filter_key: "Name",
      data: [
        { Code: "Standard", Name: "Cửa hàng trả hàng" }
      ],
    };
  ...
  initTable() {
    this.tableConfig = {
      ...
      columns: {
        ...
        options: [
          {
            title: "FinishPo.SORCode",
            name: "SORCode",
            link: true,
            newpage: true,
            onClick: (data: any) => {
              return `/${this.rootPath}/sor/details/${data.POCode}`;
            },
            style: {
              "min-width": "160px",
              "max-width": "160px",
          ...
          {
            title: "FinishPo.SORType",
            name: "POType",
            render: (data: any) => {
              return this.translate.instant(`SORType.${data.POType}`);
            },
            // style: {
            //   "min-width": "90px",
            //   "max-width": "90px",
          ...
          },
          {
            ...
            name: "SORStatus",
            render: (data: any) => {
              return this.translate.instant(`SORStatus.${data.Status}`);
            },
          },
          {
            ...
            onClick: (row: any) => {
              if (row.Status == PO_STATUS.New) {
                return `${this.rootPath}/sor/receive-pallet/${row.POCode}`;
              }
            },
            showConfirm: (row: any) => {
    ...
    this.appTable["actionEvent"].subscribe({
      next: (event: any) => {
        ...
        switch (action) {
          ...
          case "upload-doc":
            window.open(
              `/${this.rootPath}/sor/details/${event.data["POCode"]}#document`,
              "_blank"
            );
            break;
          case "print-doc":
            window.open(
              `/${this.rootPath}/sor/details/${event.data["POCode"]}#receive`,
              "_blank"
            );
            break;
  ...
  showConfirmReceive(data: any) {
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
        window.open(
          `/${this.rootPath}/sor/receive-pallet/${data.POCode}`,
          "_blank"
        );
      }
    });
