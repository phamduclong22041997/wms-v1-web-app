...
export class SORDetailsComponent implements OnInit, AfterViewInit {
  POCode: string;
  tableConfig: any;
  ...
  constructor(
    ...
    this.POCode = this.route.snapshot.params.Code;
  ...
  initReceiveTableAction(): TableAction[] {
    return [
      ...
      },
      {
        ...
        disabledCondition: (row: any) => {
          return !row.FinishedDate && ![RECEIVING_STATUS.New, RECEIVING_STATUS.Canceled].includes(row.Status);
        }
      },
      {
        icon: "print",
      ...
      },
      {
        ...
        class: 'ac-remove',
        toolTip: {
          name: "Hủy phiên nhận hàng",
        },
        disabledCondition: (row: any) => {
          return row.Status == RECEIVING_STATUS.New;
        }
      },
    ];
  }
        ...
        options: [
          {
            ...
            newpage: true,
            onClick: (row: any) => {
              return `/${this.rootPath}/product/${this.data.ClientCode}/${this.data.WarehouseSiteId}/${row.SKU}`;
            }
          },
          {
      ...
      .subscribe((resp: any) => {
        if (resp.Data) {
          this.data = {
            ...
            StoreCode: resp.Data.StoreCode,
            StoreName: `${resp.Data.StoreCode} - ${resp.Data.StoreName}`,
            ...
            POType: this.translate.instant(`SORType.${resp.Data.POType}`),
  ...
  cancel() {
    this.router.navigate([`/${window.getRootPath(true)}/sor/list`]);
  }
      ...
      next: (event: any) => {
        ...
        switch (action) {
          ...
          case 'finish-receiving':
            ...
            break;
          case "cancel":
            this.showConfirmCancelReceiveSession(event.data.SessionCode);
            break;
        }
      }
    });
  ...
  showConfirmCancelReceiveSession(code: string) {
    const dialogRef = this.dialog.open(NotificationComponent, {
      data: {
        message: `Bạn có chắc chắn muốn hủy phiên nhận hàng: ${code} này?`,
        type: 1
      }
    });
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        this.processCancelReceiveSession(code);
      }
    });
  }
  processCancelReceiveSession(code: string) {
    this.service.cancelReceiveSessionCode({ Code: code }).subscribe((resp) => {
      if (resp && resp.Status && resp.Data) {
        this.toast.success(`Hủy phiên ${code} thành công thành công`, 'success_title');
        this.loadReceiveList();
      } else {
        this.toast.error(`Hủy phiên không thành công. Vui lòng kiểm tra lại!`, 'error_title');
      }
    });
  }
  ...
  repairDataPO(dataSession: any, receiveSession: any = []) {
    ...
    if (receiveSession && receiveSession.length) {
      ...
      if (Object.keys(sku).length) {
        for (let item in sku) {
          let podetails = dataPO.Details.filter(x => x.SKU == item);
          if (podetails.length) {
