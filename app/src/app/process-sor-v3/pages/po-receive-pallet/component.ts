...
export class SORReceiptPalletComponent implements OnInit, AfterViewInit {
  ...
  type: string;
  inputPlaceholder: string = 'SOR.ContentHolder';
  client = '';
  showScan: boolean;
  ...
  confirmBarcodeQty(code: any) {
    ...
    let product = this.selectedProduct || this.data['Products'].find(x => x.SKU == info.SKU && x.BaseQty > 0);
    if (!product) {
      this.toast.error(`Không tìm thấy sản phẩm hợp lệ cho barcode này: [${code}]`, 'error_title');
      return;
    }
    isPromotion = isPromotion && (product && product.PromotionRef);

    if (isPromotion) {
      let _product = this.selectedProduct || this.data['Products'].find(x => x.SKU == info.SKU && x.PromotionRef && x.BaseQty > 0);
      if (_product && _product.BaseQty < _product.OriginalQty) {
  ...
  resetPage() {
    if (!this.code) {
      ...
      this.router.navigate([`/${window.getRootPath(true)}/sor/receive-pallet`]);
    }
  }
  ...
  scanPO(code: string, scheduleDate: string = "", lineItem: string = "") {
    if(code && !code.startsWith("SOR")){
      this.toast.error(`SOR.Message_Error_Scan_SOR`, 'error_title');
      return;
    }
    this.service.scanSOR(code, scheduleDate, lineItem)
      .subscribe((resp: any) => {
        if (resp.Status === true) {
