...
@Component({
  selector: 'app-usingstatus-combo',
  templateUrl: './usingstatus.component.html',
  styleUrls: ['./usingstatus.component.css'],
  inputs: ['title', 'tabindex', 'value', 'disabled'],
  outputs: ['change:onChange']
})
export class UsingStatusComboComponent implements OnInit {
  ...
  optionSelected(selectedVal) {
    ...
    for(let i = 0; i < this.data.length; i++) {
      if(this.data[i]['name'] == this.selectedValue) {
        _data = this.data[i];
        break;
      }
    }
  ...
  private loadData() {
    this.networkingService.get('TransportDevice.usingstatus', {})
    .subscribe(resp=>{
      let _data = [];
