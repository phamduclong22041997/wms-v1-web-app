...
@Component({
  selector: 'app-floor-combo',
  templateUrl: './floor.component.html',
  styleUrls: ['./floor.component.css'],
  inputs: ['title', 'tabindex', 'value', 'disabled'],
  outputs: ['change:onChange']
})
export class FloorComboComponent implements OnInit {
  ...
  optionSelected(selectedVal) {
    ...
    for(let i = 0; i < this.data.length; i++) {
      if(this.data[i]['floorname'] == this.selectedValue) {
        _data = this.data[i];
        break;
      }
    }
  ...
  private loadData() {
    this.networkingService.get('Floor.floorcombo', {})
    .subscribe(resp=>{
      let _data = [];
