...
@Component({
  selector: 'app-transportdevicetype-combo',
  templateUrl: './transportdevicetype.component.html',
  styleUrls: ['./transportdevicetype.component.css'],
  inputs: ['title', 'tabindex', 'value', 'disabled'],
  outputs: ['change:onChange']
})
export class TransportDeviceTypeComboComponent implements OnInit {
  ...
  optionSelected(selectedVal) {
    ...
    for(let i = 0; i < this.data.length; i++) {
      if(this.data[i] && this.data[i]['type'] == this.selectedValue) {
        _data = this.data[i];
        break;
      }
    }
  ...
  private loadData() {
    this.networkingService.get('TransportDevice.transportdevicetype', {})
    .subscribe(resp=>{
      let _data = [];
