...
export class SmartcartComboComponent implements OnInit {
  ...
  optionSelected(selectedVal) {
    ...
    for(let i = 0; i < this.data.length; i++) {
      if(this.data[i]['smartcartcode'] == this.selectedValue) {
        _data = this.data[i];
        break;
      }
    }
  ...
  private loadData() {
    this.networkingService.get('Smartcart.getcombo', {})
    .subscribe(resp=>{
      let _data = [];
