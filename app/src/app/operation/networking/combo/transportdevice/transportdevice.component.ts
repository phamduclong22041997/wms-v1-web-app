...
@Component({
  selector: 'app-transportdevice-combo',
  templateUrl: './transportdevice.component.html',
  styleUrls: ['./transportdevice.component.css'],
  inputs: ['title', 'tabindex', 'value', 'disabled'],
  outputs: ['change:onChange']
})
export class TransportDeviceComboComponent implements OnInit {
  ...
  optionSelected(selectedVal) {
    ...
    for(let i = 0; i < this.data.length; i++) {
      if(this.data[i]['transportdevicecode'] == this.selectedValue) {
        _data = this.data[i];
        break;
      }
    }
  ...
  private loadData() {
    this.networkingService.get('TransportDevice.getcombo', {})
    .subscribe(resp=>{
      let _data = [];
