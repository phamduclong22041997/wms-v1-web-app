...
export class ViewSTOListComponent implements OnInit, AfterViewInit {
  ...
  DataStatusSAP = [
    { Code: '', Name: 'All' },
    { Code: 'Y', Name: 'THÀNH CÔNG CÓ DOID' },
    { Code: 'N', Name: 'THÀNH CÔNG CHƯA CÓ DOID' },
    ...
    this.statusConfig = {
      ...
      render: (option: any) => {
        if (this.dataStatus.length > 0) {
          // return option['Name'];
          return this.translate.instant(`ViewRocketPlanning.RocketSTOStatus.${option['Name']}`);
        }
        return null;
      ...
      filter_key: 'Name',
      data: [
        // { Code: 'New', Name: 'ĐANG CHỜ TẠO STO' },
        // { Code: 'Create', Name: 'ĐÃ TẠO STO' },
      ]
    };
  ...
  loadData(filter: any) {
    this.service.getSTOFromRocket(filter)
      .subscribe((resp: any) => {
        if (resp['Status'] && resp['Data']) {
          ...
          this.soList = resp['Data'] ? resp['Data'] : [];
          this.appTable['renderData'](this.soList);

          for (let idx in this.soList) {
            this.soList[idx]['Status'] = this.translate.instant(`ViewRocketPlanning.RocketSTOStatus.${this.soList[idx]['Status']}`);
          }
        } else {
          this.resetData();
        }
    ...
    this.tableConfig = {
      ...
      columns: {
        ...
        options: [
          ...
          {
            title: 'ViewRocketPlanning.SOCode',
            name: 'SOCode',
            link: true,
            newpage: true,
            onClick: (data: any) => {
              return `/${window.getRootPath()}/purchaseorder/details/${data.STOCode}`;
            },
            style: {
              'min-width': '180px',
              'max-width': '180px'
  ...
  goToPageImportSTO(event: any = null) {
